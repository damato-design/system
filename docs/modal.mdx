...
If these didn't cover your use case and creativity for an alternative solution is lacking, then here's an example that uses the `Dialog` component to launch onto the `top-layer`. This keeps it as a separate context from the underlying page.
