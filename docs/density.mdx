...
>
> The stories found for most of the components here are displayed with no change in density. This means they are displayed at their largest size. In order to reduce the size of any component, it must exist in a container where `denser` was applied or the `mode="system:denser"`.
...
[mode~="system:denser"] [mode~="system:denser"] [mode~="system:denser"] {
     --level: 0;
}
