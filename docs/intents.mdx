...
Note that the concept of `foregroundColor` is meant to cover **text and icons**. You can consider both of these collections of glyphs which align to having the same color between them. This is similar to setting `<svg fill="currentColor"/>` to inherit the text color in code.
