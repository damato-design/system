...
Where `level` is determined from [the amount of depth changes](/docs/foundations-density--docs) expected to occur in the experience. This means at the most compact parts of the experience (`level: 0`), the resulting font size will be `minRem * 1rem`. Each level up will scale up the size by the given `scaleFactor`. Here's what this would look like with `minRem: 1`, `scaleFactor: 1.125`, with 4 levels of depth:
