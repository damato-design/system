...
The `brand` option determines what brand to include in the page by Without setting this value, the page would be presented as unbranded, using system and symbolic colors. Only one brand can be loaded. Brands omitted from this list will not be lazy loaded.

> #### What if I want to support multiple brands?
